...
if ! cd /home/multichain; then
	mc_log "[EXPLORER][WARN] Failed to cd to /home/multichain"
fi
...
attempt=0
ip=""
# Allow MASTER_PORT to be a comma/space separated list
MASTER_PORT_LIST=$(echo "${MASTER_PORT}" | tr ',;' ' ')
while [[ $attempt -lt $RETRIES ]]; do
	if ip=$(resolve_master) && [[ -n $ip ]]; then
		for p in $MASTER_PORT_LIST; do
			p_trim=$(echo "$p" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
			[[ -z "$p_trim" ]] && continue
			if (echo >/dev/tcp/$ip/$p_trim) >/dev/null 2>&1; then
				mc_log "[EXPLORER] Master reachable at $ip:$p_trim"
				MASTER_PORT=$p_trim
				break 2
			fi
		done
...
if [[ "$GENERATE_EXPLORER_CONF" == "1" ]]; then
	...
	} > "$tmp_ini" && mv "$tmp_ini" "$EXPLORER_CONF"
...
while [[ $rpc_try -lt $max_rpc_tries ]]; do
	...
	if [[ -n "${RPC_USER:-}" && -n "${RPC_PASSWORD:-}" ]]; then
		resp=$(curl -sS --max-time 5 -u "${RPC_USER}:${RPC_PASSWORD}" -X POST -H 'content-type: text/plain;' --data-binary '{"jsonrpc":"1.0","id":"hc","method":"getinfo","params":[]}' "${SERVER_RPC_URL}" 2>/dev/null || true)
	else
		resp=$(curl -sS --max-time 5 -X POST -H 'content-type: text/plain;' --data-binary '{"jsonrpc":"1.0","id":"hc","method":"getinfo","params":[]}' "${SERVER_RPC_URL}" 2>/dev/null || true)
	fi
